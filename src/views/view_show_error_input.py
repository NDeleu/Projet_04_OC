...
class ShowErrorInput:
    ...
    def error_choice_input():
        return "ERREUR : Cette entrée n'est pas attribuée à " \
               "une action possible ou existante.\n" \
               "Merci de saisir un numéro relatif à " \
               "une action proposée ci dessus :"

    error_choice_input = staticmethod(error_choice_input)

    def error_value_num_input():
        return "ERREUR : Cette entrée ne correspond pas au " \
               "format attendu. \n" \
    ...
    def error_value_alphanum_input():
        return "\nERREUR : Cette entrée ne correspond pas au " \
               "format attendu. \n" \
    ...
    def error_already_exists(type_item, name_instance):
        return f"\nERREUR : Les données : {type_item} : {name_instance} " \
               f"existe déjà dans la base de donnée. \n" \
    ...
    def error_not_exists(type_item, name_instance):
        return f"\nERREUR : Les données {type_item} : {name_instance} " \
               ...
               f"Vous pouvez à cette fin consulter " \
               f"la base de donnée depuis le menu principal \n"

    error_not_exists = staticmethod(error_not_exists)

    def error_not_enough_characters(len_min):
        return f"\nERREUR : Cette entrée ne possède pas " \
               f"assez de caractères. \n" \
               f"Merci de saisir une entrée avec pour minimum : {len_min} " \
               f"caractères \n"
    ...
    def error_too_many_characters(len_max):
        return f"\nERREUR : Cette entrée possède trop de caractères. \n" \
               f"Merci de saisir une entrée avec pour maximum : {len_max} " \
               f"caractères \n"
    ...
    def error_id_player_input():
        return "\nERREUR : Cette entrée ne correspond pas au " \
               "format attendu.\n" \
               "Merci de saisir des chiffres pour les deux premiers " \
               "caractères, \n" \
    ...
    def error_record():
        return "Une erreur est survenue lors de la création et " \
               "l'enregistrement des données. \n" \
               "Veuillez réitérer l'opération. Si le problème persiste, " \
    ...
    def error_tournament_name(tournament_name):
        return f"Le tournoi nommé {tournament_name} n'est pas reconnu. \n" \
               f"Vous pouvez consulter la base de donnée depuis " \
               f"le menu principal pour " \
               f"vous assurer de l'existance ou de " \
               f"l'orthographe du nom du tournoi."
