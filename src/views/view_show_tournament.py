...
class ShowTournament:
    ...
    def init_load_tournament(len_min, len_max):
        return f"Veuillez saisir le nom du tournoi que vous souhaitez charger. \n" \
               f"Il doit être composé au minimum de {len_min} caractères et au maximum de {len_max} caractères"

    init_load_tournament = staticmethod(init_load_tournament)

    def show_load_id_player_to_tournament():
        return "Veuillez saisir l'identifiant du joueur que vous souhaitez intégrer au tournoi. \n" \
    ...
    show_load_id_player_to_tournament = staticmethod(show_load_id_player_to_tournament)

    def show_choice_id_player_to_tournament():
        return "Désirez vous : \n" \
               "Charger un joueur préexistant dans la base de donnée et l'ajouter au tournoi :  1 \n" \
               "Créer un joueur dans la base de donnée et l'ajouter au tournoi :                2"

    show_choice_id_player_to_tournament = staticmethod(show_choice_id_player_to_tournament)
    ...
    show_error_check_player_in_player_list_tournament = staticmethod(show_error_check_player_in_player_list_tournament)
    ...
    show_not_enough_player_in_list_player = staticmethod(show_not_enough_player_in_list_player)

    def show_completed_player_in_list_player():
        return "Les 8 joueurs participants ont étés enregistrés pour ce tournoi. \n" \
               "Voici la liste des joueurs participants : \n"

    show_completed_player_in_list_player = staticmethod(show_completed_player_in_list_player)

    def show_all_player_in_list_player(list_player):
        for player in list_player:
            print(player)

    show_all_player_in_list_player = staticmethod(show_all_player_in_list_player)
