...
class Round:
    def __init__(self, liste_joueur, tours_round):
    ...
    def round_start(self):
        if "round{}".format(self.check_round) not in self.round_match :
            ...
            if self.check_match < int(len(self.liste_joueur) / 2):
                print("Vous n'avez pas fini le round en cours, veuillez saisir le résultat de tous les matchs du round en cours")
            else :
                self.check_round +=1
                self.check_match = 0
                self.round_start()
    ...
    def first_round(self):
        ...
        for y in range(int(len(sorted_by_classement)/2)):
            self.round_match["round{}".format(self.check_round)].append((sorted_by_classement[y], sorted_by_classement[y+(int(len(sorted_by_classement)/2))]))
    ...
    def other_round(self):
        ...
        while int(len(self.round_match["round{}".format(self.check_round)]) < int(len(self.liste_joueur)/2)) :
            if (sorted_by_roundpoint[y], sorted_by_roundpoint[y+1]) or (sorted_by_roundpoint[y+1], sorted_by_roundpoint[y]) not in self.round_match :
                self.round_match["round{}".format(self.check_round)].append((sorted_by_roundpoint[y], sorted_by_roundpoint[y+i]))
